 ...
 */
class importDipObjectsTask extends sfBaseTask
{
  protected
...
EOF;
  ...
  public function execute($arguments = array(), $options = array())
  {
    ...
    sfConfig::set('app_upload_dir', $this->getUploadDir());
    ...
    {
      $undoLog = rtrim($options['undo-log-dir'], '/') .'/'. date('Y-m-d') .'-'. basename($this->dipDir);
    }
    ...
    $digitalObjects = $this->parseCsvData($this->openFirstCsvFile(), $objectsPath);
    $count = $this->importDigitalObjects($digitalObjects, $options['audit']);
   ...
   */
  protected function openFirstCsvFile()
  {
    // Find all CSV files in the objects directory
    $csvFiles = sfFinder::type('file')->name('*.csv')->in($this->dipDir .'/objects');
  ...
  protected function parseCsvData($fh, $objectsPath)
  {
    ...
    while ($row = fgetcsv($fh, 1000))
    {
      ...
      $filepath = rtrim($objectsPath, '/') .'/'. $filename;
  ...
  protected function processCsvHeaderRow($fh)
  {
    ...
    if (in_array('identifier', $this->columnNames) && in_array('slug', $this->columnNames))
    {
   ...
   * @return void
   */
  protected function importDigitalObjects($digitalObjects, $auditMode = false)
  {
    $count = 0;
    ...
    foreach ($digitalObjects as $key => $item)
    {
      ...
      if ($auditMode)
      {
        ...
        else
        {
          ...
          $parent = $this->getInformationObjectUsingUniqueId($key);

          for ($i = 0; $i < count($item); $i++)
          {
            $this->auditDigitalObject($item[$i]);
            $count++;
          }
        }
      }
      else
      {
        ...
        else
        {
          ...
          for ($i = 0; $i < count($item); $i++)
          {
            ...
            $this->addDigitalObject($informationObject, $item[$i], $undoLog, true);
            $count++;
          }
        }
      }
  ...
  {
    if (!($files = scandir($objectsPath)))
    {
      throw new sfException("Couldn't read path '$objectsPath'");
    }
   ...
   * string the first found is returned. Boolean false is returned if no UUID
   * is found.
   *
