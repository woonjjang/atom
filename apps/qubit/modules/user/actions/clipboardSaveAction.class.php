...
class UserClipboardSaveAction extends sfAction
{
  public function execute($request)
  {
    ...
    else
    {
      ...
      {
        $message = $this->context->i18n->__('Password generation failure. Please try again.');
        $this->context->user->setFlash('error', $message);
      }

      $this->saveClipboard($allSlugs, $password);

      $message = $this->context->i18n->__('Clipboard saved. Password is <b>%1%</b>. Please write this down.', array('%1%' => $password));
